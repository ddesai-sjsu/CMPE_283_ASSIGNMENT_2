...
int kvm_emulate_cpuid(struct kvm_vcpu *vcpu)
{
	...
	if(eax == 0x4fffffff){
        ...
        printk(KERN_INFO "Exit time=%llu", atomic64_read(&exits_time));
		
        // Return the high 32 bits of the total time spent processing all exits in %ebx
		ebx = (atomic64_read(&exits_time) >> 32);
		printk(KERN_INFO "High 32 bits of the total time spent processing all exits in %ebx=%u", ebx);


		// Return the low 32 bits of the total time spent processing all exits in %ecx
		ecx = (atomic64_read(&exits_time) & 0xFFFFFFFF);
		printk(KERN_INFO "Low 32 bits of the total time spent processing all exits in %ecx=%u", ecx);
